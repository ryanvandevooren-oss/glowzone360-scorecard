-- ═══════════════════════════════════════════════════════════════
-- 037_scorecard_winner_name.sql
-- Winner identity end-to-end, part 1 (DB):
--   1. games.player_names        (jsonb, nullable, index-aligned with scores)
--   2. secret_score_wins.winner_name (text snapshot, nullable)
--   3. evaluate_secret_score — capture ALL matching players,
--      join names with ' & ', return winner_name in the verdict
-- Additive; old rows keep NULLs and all displays degrade gracefully.
-- ═══════════════════════════════════════════════════════════════
-- ───────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────
-- 2. Engine — evaluate_secret_score
--    (based on the LIVE definition pulled 2026-07-10; changes
--     marked with -- [037] comments)
-- ───────────────────────────────────────────────────────────
...
declare
  ...
  v_scores         jsonb;
  v_pars           int[];
  v_names          jsonb;   -- [037] player_names from the game row
  v_enabled        boolean;
  v_cap            int;
  v_reveal         text;
  v_pool_mode      text;
  ...
  v_tier           text;
  v_match          boolean;
  v_match_count    int;     -- [037]
  v_winner_name    text;    -- [037] joined names of ALL matching players
...
begin
  ...
  returning g.location_id, g.scores, g.pars_snapshot, g.player_names   -- [037]
       into v_loc, v_scores, v_pars, v_names;                          -- [037]
  ...
  if not found then
    ...
    if found then
      return jsonb_build_object(
        ...
        'winner_name', v_win.winner_name,        -- [037]
  ...
  -- [037] replaced the anonymous exists() with a match that also
  -- collects the names of ALL matching players (by index alignment
  -- between scores and player_names). Names are trimmed and capped
  -- at 40 chars defensively; NULL/missing names are skipped, so
  -- pre-037 rows (player_names IS NULL) match exactly as before
  -- but produce winner_name = NULL.
  select count(*),
  ...
  for i in 1..12 loop
    begin
      ...
      insert into scorecard.secret_score_wins(
        ...
        prize_name_snapshot, redeem_code, winner_name)             -- [037]
      values (v_loc, p_game_id, v_hole, v_score, v_prize_id,
              v_prize_name, v_code, v_winner_name);                -- [037]
      exit;
